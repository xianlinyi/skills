...
- **Always declare an `allowed-tools` list in the SKILL.md frontmatter** for every new skill. Enumerate only the tools the skill actually invokes (e.g. `bash`, `create`, `edit`, `view`, `glob`, `grep`, `ask_user`). Never grant tools not needed by the skill's steps.
